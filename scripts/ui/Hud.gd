...
func refresh() -> void:
	...
	if state.run_status == "victory":
		...
		tutorial.text = "Это развернутая поверхность реактора. A/D шагают по ленте, W/S меняют дорожку, Q/E или мышь прокручивают реактор."
